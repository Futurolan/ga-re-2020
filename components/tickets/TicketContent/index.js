...
function TicketContent ({ data: { loading, error, node } }) {
  if (error) {
    return (
      <div className='notification is-danger'>Une erreur est survenue pendant le chargement de la billetterie !!!
      </div>
    )
  }
  ...
  if (node && node.url && node.weezeventId) {
    return (
      <div className='ga-ticket-content '>
            ...
            {node.url2 && config.tickets.pass &&
              <div className='box content'>
                ...
                <div className='has-text-centered'>
                  <a target='_blank' rel='noopener noreferrer' href={node.url2}><button className='button is-primary is-fullwidth'>{config.tickets.pass.button}</button></a>
                </div>
              </div>}
            {node.minor && config.tickets.minor &&
              <div className='box content'>
                ...
                <div className='has-text-centered'>
                  <a target='_blank' rel='noopener noreferrer' href={node.minor.file.url}><button className='button is-primary is-fullwidth'>{config.tickets.minor.button}</button></a>
                </div>
              </div>}
            {node.rules && config.tickets.rules &&
              <div className='box content'>
                ...
                <div className='has-text-centered'>
                  <a target='_blank' rel='noopener noreferrer' href={node.rules.file.url}><button className='button is-primary is-fullwidth'>{config.tickets.rules.button}</button></a>
                </div>
              </div>}
          </div>
        </div>
